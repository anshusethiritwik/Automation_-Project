...
if [ $(systemctl status apache2 | grep -v grep | grep 'apache2 is running' | wc$
then
        echo "running"
else
...
if [[ -f /tmp/${name}-htttpd-logs-${timestamp}.tar ]];
then
        aws s3 cp /tmp/${name}-httpd-logs-${timestamp}.tar s3 ://${s3_bucket}/${name}--https-logs-${timestamp}.tar
fi
